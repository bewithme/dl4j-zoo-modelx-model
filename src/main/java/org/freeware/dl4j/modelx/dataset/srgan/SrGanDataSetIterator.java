package org.freeware.dl4j.modelx.dataset.srgan;
 ...
 */
@Slf4j
public class SrGanDataSetIterator implements MultiDataSetIterator {
	...
	private NativeImageLoader nativeImageLoaderHr ;
	...
	public SrGanDataSetIterator(String dataSetPath, int batchSize, int imageHeight, int imageWidth, int channels, int imageHeightHr, int imageWidthHr, int channelsHr) {

		super();

        this.nativeImageLoader=new NativeImageLoader(imageHeight,imageWidth,channels);

		this.nativeImageLoaderHr=new NativeImageLoader(imageHeightHr,imageWidthHr,channelsHr);
      ...
      //检查并设置目录
		...
		//获取所有特征数据
    	this.featureFiles = ExtendedFileUtils.listFiles(featurePath, new String[] {"jpg","png"}, Boolean.TRUE);
	...
	public MultiDataSet next(int batchSize) {
		...
		for(int exampleIndex=startIndex;exampleIndex<endIndex;exampleIndex++) {
			...
			File labelFile=featureFiles.get(exampleIndex);
			
			try {
				//得到图片特征[N,C,H,W]
				INDArray imageFeature = nativeImageLoader.asMatrix(featureFile);

				INDArray label = nativeImageLoaderHr.asMatrix(labelFile);
